...
export class InterestRateSimpleService {
  ...
  calculatorInteresRateSimple(
    ...
    if (finalValue != null && presentValue != null) {
      ...
      if (timePeriodo > 0) {
        resultInterestRatePeriodo = ((finalValue / presentValue - 1) / timePeriodo) * 100;
      }
    }
    if (presentValue != null && interes != null) {
      resultInterestRateAnual = (interes / (presentValue * timeAnual)) * 100;
      if (timePeriodo > 0) {
        resultInterestRatePeriodo = (interes / (presentValue * timePeriodo)) * 100;
      }
    }

    if (finalValue != null && interes != null && presentValue === 0) {
      resultPresentValue = finalValue - interes;
      resultInterestRateAnual = (interes / (resultPresentValue * timeAnual)) * 100;
      if (timePeriodo > 0) {
        resultInterestRatePeriodo = (interes / (resultPresentValue * timePeriodo)) * 100;
      }
    }

    resultadoTiempoFinal = timePeriodo > 0 ? timePeriodo : timeAnual;
    resultadoInteresFinal = resultInterestRatePeriodo != 0 ? resultInterestRatePeriodo : resultInterestRateAnual;
    let mensaje: any;
    if (timePeriodo > 0) {
